"""Stage F: deterministic normalization — dates, times, categories, legacy + rich rows."""
...
def normalize_time_to_hhmm(value: str | None) -> str | None:
    """Accept 24h (17:30), 12h (5:30 PM / 5:30PM), and normalize to HH:mm."""
    if value is None:
        return None
...
def normalize_candidates(raw_candidates: List[dict]) -> List[EventCandidate]:
    """Legacy flat rows from monolithic LLM (v1)."""
    ...
    for item in raw_candidates:
        ...
        start_value = normalize_time_to_hhmm(start_raw)
        end_value = normalize_time_to_hhmm(end_raw)
...
def normalize_end_date(value: Optional[str]) -> Optional[str]:
    v = _clean_optional(value)
    return v if v and _is_valid_date(v) else None
