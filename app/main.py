...
def watch_seen(payload: WatchSeenRequest) -> dict:
    started = time.perf_counter()
    try:
        watch_st = (payload.watch_state or "").strip().lower()
        if watch_st in {"not_started", "watching", "finished"}:
            success, message = set_watch_progress(payload.device_id, payload.show_id, watch_st)
            api_seen = watch_st == "finished"
        else:
            ...
            api_seen = bool(payload.seen)
            watch_st = "finished" if api_seen else "not_started"
        ...
        return {
            ...
            "seen": api_seen,
            "watch_state": watch_st if success else "",
        }
    except Exception as exc:
