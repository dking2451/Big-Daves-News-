...
@app.get("/api/brief-narration")
def brief_narration(topics: str | None = None, device_id: str | None = None) -> dict:
    """Returns a personalized 3-4 sentence morning brief generated by Claude.

    Query params:
      topics     — comma-separated topic keys (e.g. "health,technology,science")
      device_id  — optional, reserved for future per-device personalization
    """
    started = time.perf_counter()
    try:
        preferred: list[str] | None = None
        if topics:
            preferred = [t.strip().lower() for t in topics.split(",") if t.strip()]
        ...
        claims = select_relevant_headlines(claims, per_topic_limit=5, total_limit=40)

        try:
            narration = generate_daily_brief_narration(claims, preferred_topics=preferred)
        except Exception:
            narration = fallback_daily_brief_narration(claims, preferred_topics=preferred)

        _record_api_metric("brief_narration", int((time.perf_counter() - started) * 1000), True)
        return {"narration": narration, "generated_at": __import__("datetime").datetime.utcnow().isoformat()}
    except Exception as exc:
        _record_api_metric("brief_narration", int((time.perf_counter() - started) * 1000), False, str(exc))
        return {"narration": "", "generated_at": ""}
